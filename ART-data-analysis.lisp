...
;;
;; NOTE: ENTIRE FILE MAY ONLY APPLY WHEN USE ARRAYS TO STORE ART DATA DURING PROCESSING



;;ART POST-RUNART  DATA ANALYSIS LIST STRUCTURES -------------------
;;
;; 1. EACH CELL-SYM (eg. input1) IS SET TO A LIST = (root dims n-elements value)
;; MUST APPEND EACH WITH A NEW LIST  (celln (x-axis-pixel value) "label")
;;old CL-USER 61 > input1= ;;(1 (80 0.040319994))
;;
...
;; 
;; NEW?  (getsymvals 'input :val-n 5) 
;; OLD 3. (make-points-list input) RETURNS GRAPHING VALUES 
...
;;(setf  *art2-instance (make-instance 'ART-multipane-interface))
...
;;CONVERT-ART-ARRAYS-TO-LISTS
;;
;;ddd
(defun convert-ART-arrays-values-to-lists (array-list symbol-root-list 
     ...
     (loop
      ...
      (dotimes ( j-dim n-dims)  ;;j-dim becomes 1 or 2
        ...
        (multiple-value-setq (converted-arrays-list new-symbols-list) ;was new-symbol
            (convert-ART-array-values-to-list  array  
...
(defun testca ()
      (let*
        ...
        (convert-ART-arrays-values-to-lists array-list symbol-root-list
...
;; REPLACE WITH REAL OUTPUT FROM ART?
...
(defun convert-ART-array-values-to-list (array 
  ...
  (let*
    ...
    ;;end let, convert-ART-array-values-to-list
    ))
;;TEST 
(defun testca1 ()
  (let*
  ...
  (multiple-value-bind (converted-arrays-list symbol)
      (convert-ART-array-values-to-list  array1 3 :symbol-root symbol :add-label-p add-label-p)   
...
(defun testca2 ()
  ...
  (multiple-value-bind (converted-arrays-list symbol-list)
      (convert-ART-array-values-to-list  array1  :symbol-root symbol :add-label-p add-label-p)   
...
#|(defun ART2 (&key (plot? nil))   
  ...
  (initNetwork)
  (learnPatterns **n-inputs **n-outputs *testPatterns *learnPatterns-cycles)
)|#
