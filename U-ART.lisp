...
(defun find-sym-dims (artsym  &key (separator *art-index-separator)
  ...
  (let*
    ...
    (cond
     ...
     ;;for first dim letter at end of rootstr
     ((and (null dims-found-p)(string-equal (string (char string (+ n 1))) separator))
...
(defun make-one-dim-indecies (dimspec) ;;no &key index-syms *art-index-syms)
  "U-ART, Makes the dims for ONE dim based upon its dimspec. eg for (4  2  3)) = (2 5 8 11) [n=4, begins at 2, incr= 3)"
...
;;
;;ddd
(defun make-dim-symbol (root dimlist &key (separator-str "-") tail-string return-only-str-p (set-to-symvals-if-unbound-p T) value subsyms)
  "In U-ART, Makes a new symbol new-dimsym. If set-to-symvals-if-unbound-p sets new sym to (root dimlist nil value subsyms).  If  RETURNS (values new-dimsym symdims-str symvals) eg WUPI-3-2TOI-1-3   \"WupI-3-2TOI-1-3\"). WILL NOT change values or subsyms if bound.  If bound, returns old value and old subsyms"
  (let
      ((symdims-str)
       (new-dimsym)
...
(defun incf-artdims (cur-dims &rest dimsyms)
  "In U-ART, adds 1 to the all dims indicated by dimsym (I, L F or M) or with colon."
  (let
      ((newdims)
       )
    (loop
     for dim in cur-dims
     for n from 1 to (list-length cur-dims)
     do
     (setf nth (- n 1))
     (cond
      ((and (= n 1)(intersection '(I :I) dimsyms))
       (setf newdims (incf-artdim  dim newdims)))
     ((and (= n 2)(intersection '(L :L) dimsyms))
       (setf  newdims (incf-artdim  dim newdims)))
     ((and (= n 3) (intersection '(F :F) dimsyms))
       (setf  newdims (incf-artdim  dim newdims)))
     ((and (= n 4) (intersection '(M :M) dimsyms))
       (setf  newdims (incf-artdim  dim newdims)))
...
;;TEST
;;  (incf-artdims '(4 L 2 M) :i  :f) = (5 L 3 M)
;; (incf-artdims '(4 L 2 M)   :f)  = (4 L 3 M)
;;  (incf-artdims '(4 L 2 M)   :i  :l) = (5 L 2 M)
...
;;
;;ddd
(defun incf-artdim (dim newdims)
  "In U-ART, used by incf-artdims"
  (cond
   ((numberp dim)
    (setf newdims (append newdims (list (incf dim)))))
   (t (setf newdims (append newdims (list dim))))))
;;tested in function above
